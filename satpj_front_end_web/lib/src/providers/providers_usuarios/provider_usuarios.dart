...
class ProviderUsuarios {
  static Future<List<Usuario>> obtenerUsuarioPorDocumento(
    ...
    try {
      final resp = await http.get(
        Uri.http(ApiDefinition.url, _path, {"documento": documento}),
  ...
  static Future<List<Usuario>> obtenerUsuarioPorEmail(String email) async {
    final _completer = Completer<List<Usuario>>();

    try {
      final resp = await http.get(
        Uri.http(ApiDefinition.url, _path, {"email": email}),
        headers: ApiDefinition.headerWithoutAuthorization,
      );
